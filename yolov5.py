...
import time
def yolo(image):
    model = torch.hub.load('ultralytics/yolov5', 'custom', path='fruit.pt')
    model.conf = 0.6
    model.iou = 0.6
    img1 = image[..., ::-1]
    results = model(img1)
...
img = cv2.imread('./fruit/images/test/orange.jpg')
...
# for file_name in os.listdir('./fruit/images/test'):
    ...
    # img = cv2.imread(os.path.join("./fruit/images/test", file_name))
    # corr_data = yolo(img)
    # red_corr = corr_data[corr_data['name'] == 'red']
